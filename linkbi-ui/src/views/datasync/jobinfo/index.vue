    ...
    <el-form :model="listQuery" ref="queryForm" v-show="showSearch" :inline="true">
      ...
      <el-form-item label="所属应用" prop="projectId">
        <el-select v-model="listQuery.projectId"
                   ...
                   size="small"
                   style="width: 240px">
          <el-option v-for="item in jobProjectList"
                     :key="item.id"
                     :label="item.name"
                     :value="item.id" />
        </el-select>
      ...
      <el-form-item label="任务类型" prop="glueType">
        <el-select v-model="listQuery.glueType"
                   placeholder="任务类型"
                   clearable
                   size="small"
                   style="width: 240px">
        ...
        <template slot-scope="scope">
          ...
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item @click.native="handlerExecute(scope.row)">执行一次</el-dropdown-item>
              <el-dropdown-item @click.native="handlerViewLog(scope.row)">查询日志</el-dropdown-item>
              <el-dropdown-item divided @click.native="handlerUpdate(scope.row)">编辑</el-dropdown-item>
              <el-dropdown-item @click.native="handlerDelete(scope.row)">删除</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="1000px" :before-close="handleClose">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="110px">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="所属应用" prop="projectId">
              <el-select v-model="temp.projectId" placeholder="所属应用" filterable
                         :filter-method="userFilter" class="filter-item">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="子任务">
              <el-select v-model="temp.childJobId" multiple filterable placeholder="子任务" value-key="id">
