<template>
  <el-form-item :label="$t('chart.filters')">
    ...
    <el-dialog :visible="visible" width="500px" :title="$t('chart.addFilters')" @close='closeDialog'>
...
import { buildFilterSentence_default } from '@/utils/buildSentence'
...
export default {
  ...
  methods: {
    ...
    handleConfirm() {
      ...
      {
          filterIndex = this.currentFilters.findIndex(item => {
    ...
    handleSubmit() {
      //const filterStrs = this.currentFilters.map(buildFilterSentence_default)
    ...
    generateFilterSentence(item) {
      return buildFilterSentence_default(item)
    },
