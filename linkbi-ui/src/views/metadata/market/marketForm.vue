<template>
  <div class="app-container">
    ...
    />
    <!-- 预览界面 -->
    <el-dialog :title="$t('common.edit')" :visible.sync="open" width="30%" top="5vh" append-to-body>
      <el-form ref="dataForm" :model="marketForm" label-position="left" label-width="100px">
        ...
        <el-form-item label="视图类型" prop="viewType">
          <el-select v-model="marketForm.viewType" filterable size="small" >
            <el-option
              v-for="dict in viewTypeOptions"
              :key="dict.dictValue"
              :label="dict.dictLabel"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="updateMarket()">
          确认
        </el-button>
      </div>
    </el-dialog>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getList() {
                this.loading = true;
                dsQueryApi.listMarket(this.listQuery).then(response => {
            ...
            /** 修改按钮操作 */
            handleEdit(row) {
                this.marketForm = Object.assign({}, row)
                this.open = true
            },
            updateMarket()
            {
                this.loading = true
                dsQueryApi.editMarket(this.marketForm).then(response => {
                    this.open = false
                    this.loading = false
                }).catch(e =>{
                    this.msgError(e);
            ...
            handleDelete(row) {
                ...
                const tableName = row.modeName;
                this.$confirm('是否确认删除表:"' + tableName + '"?', "警告", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    type: "warning"
                }).then(function() {
                    return dsQueryApi.deleteMarket({marketId});
                }).then(() => {
                    this.getList();
            ...
            handleSubcribe(row){
                const modeId = row.id
                dsQueryApi.insertSubcribe({modeId}).then(response => {
                    this.loading = false
                    if(Number(response.data) < 0){
                        this.msgError("表信息错误或已收藏该表");
                    }
                    else{
                        this.msgSuccess("成功")
                    }
