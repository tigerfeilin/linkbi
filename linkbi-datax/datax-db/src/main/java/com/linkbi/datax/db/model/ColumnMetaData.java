	...
	private void create(ColumnDescription desc) {
		...
		case java.sql.Types.NUMERIC:
			...
			if (type == java.sql.Types.DOUBLE || type == java.sql.Types.FLOAT || type == java.sql.Types.REAL) {
				...
				if (desc.getDbType() == DBTypeEnum.POSTGRESQL && type == java.sql.Types.DOUBLE && precision >= 16
						&& length >= 16) {
					precision = -1;
					length = -1;
				}
				...
				if (desc.getDbType() == DBTypeEnum.MYSQL) {
					if (precision >= length) {
						precision = -1;
						length = -1;
					}
			...
			if (desc.getDbType() == DBTypeEnum.POSTGRESQL || desc.getDbType() == DBTypeEnum.GREENPLUM) {
			...
			if (desc.getDbType() == DBTypeEnum.ORACLE) {
