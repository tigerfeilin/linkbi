package com.linkbi.datax.api.core.thread;
...
public class JobScheduleHelper {
    ...
    public void start() {
        ...
        scheduleThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                logger.info(">>>>>>>>> init datax-admin admin scheduler success.");
                ...
                while (!scheduleThreadToStop) {
                    ...
                    try {
                        ...
                        if (scheduleList != null && scheduleList.size() > 0) {
                            ...
                            for (JobInfo jobInfo : scheduleList) {
                                ...
                                if (nowTime > jobInfo.getTriggerNextTime() + PRE_READ_MS) {
                                    ...
                                    logger.warn(">>>>>>>>>>> datax-admin, schedule misfire, jobId = " + jobInfo.getId());
                                    ...
                                    logger.debug(">>>>>>>>>>> datax-admin, schedule push trigger : jobId = " + jobInfo.getId());
                        ...
                        if (!scheduleThreadToStop) {
                            logger.error(">>>>>>>>>>> datax-admin, JobScheduleHelper#scheduleThread error:{}", e);
                        }
                    } finally {
                ...
                logger.info(">>>>>>>>>>> datax-admin, JobScheduleHelper#scheduleThread stop");
            }
        });
        scheduleThread.setDaemon(true);
        scheduleThread.setName("datax-admin, admin JobScheduleHelper#scheduleThread");
        ...
        ringThread = new Thread(() -> {
            ...
            while (!ringThreadToStop) {
                ...
                try {
                    ...
                    // ring trigger
                    logger.debug(">>>>>>>>>>> datax-admin, time-ring beat : " + nowSecond + " = " + Arrays.asList(ringItemData));
                    ...
                    if (!ringThreadToStop) {
                        logger.error(">>>>>>>>>>> datax-admin, JobScheduleHelper#ringThread error:{}", e);
                    }
                }
            ...
            logger.info(">>>>>>>>>>> datax-admin, JobScheduleHelper#ringThread stop");
        });
        ringThread.setDaemon(true);
        ringThread.setName("datax-admin, admin JobScheduleHelper#ringThread");
        ringThread.start();
    }
    ...
    private void pushTimeRing(int ringSecond, long jobId) {
        ...
        logger.debug(">>>>>>>>>>> datax-admin, schedule push time-ring : " + ringSecond + " = " + Arrays.asList(ringItemData));
    }
    ...
    public void toStop() {
        ...
        logger.info(">>>>>>>>>>> datax-admin, JobScheduleHelper stop");
    }
