...
public class JobFailMonitorHelper {
			...
			public void run() {
				...
				while (!toStop) {
					try {
						...
						if (!toStop) {
							logger.error(">>>>>>>>>>> datax-admin, job fail monitor thread error:{0}", e);
						}
					}
				...
				logger.info(">>>>>>>>>>> datax-admin, job fail monitor thread stop");

			}
		});
		monitorThread.setDaemon(true);
		monitorThread.setName("datax-admin, admin JobFailMonitorHelper");
		monitorThread.start();
	}
	...
	private boolean failAlarm(JobInfo info, JobLog jobLog){
		...
		if (info!=null && info.getAlarmEmail()!=null && info.getAlarmEmail().trim().length()>0) {
			...
			// email info
			JobGroup group = JobAdminConfig.getAdminConfig().getJobGroupMapper().load(Long.valueOf(info.getJobGroup()));
			...
			for (String email: emailSet) {
				...
				try {
					...
					logger.error(">>>>>>>>>>> datax-admin, job fail alarm email send error, JobLogId:{}", jobLog.getId(), e);
