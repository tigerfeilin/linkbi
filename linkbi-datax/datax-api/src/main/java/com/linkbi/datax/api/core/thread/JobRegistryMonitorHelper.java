...
public class JobRegistryMonitorHelper {
	...
	public void start(){
		registryThread = new Thread(() -> {
			while (!toStop) {
				try {
					...
					if (groupList!=null && !groupList.isEmpty()) {
						...
						List<Long> ids = JobAdminConfig.getAdminConfig().getJobRegistryMapper().findDead(RegistryConfig.DEAD_TIMEOUT, new Date());
					...
					if (!toStop) {
						logger.error(">>>>>>>>>>> datax-admin, job registry monitor thread error:{}", e);
					}
				}
				try {
			...
			logger.info(">>>>>>>>>>> datax-admin, job registry monitor thread stop");
		});
		registryThread.setDaemon(true);
		registryThread.setName("datax-admin, admin JobRegistryMonitorHelper");
